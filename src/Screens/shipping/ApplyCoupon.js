  ...
  const CouponApplyed = async item => {
    ...
    try {
      ...
      const response = await axios.post(
        `${API.COUPON_APPLYED}`,
        {coupon_id: item.id, order_amount: Orderamount.substring(1, 14)},
        {
          headers: {Authorization: ` ${usertkn}`},
        },
      );
          ...
          <FlatList
            ...
            renderItem={({item, index}) => (
              <TouchableOpacity
                ...
                <View
                  style={{flexDirection: 'column', flex: 1, marginLeft: 25}}>
                  <View
                    style={{
                      flex: 0.6,
                      marginTop: 25,
                    ...
                    <Text
                      style={{textAlign: 'left', fontSize: 15, color: 'black'}}>
                      {item.name}
                    </Text>
                    <Text
                      style={{
